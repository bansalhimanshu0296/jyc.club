...
class Core_Service_System extends Phpfox_Service 
{
	...
	public function get()
	{
	    ...
	    (
	    	...
	    	'Mysql Replication Enabled' => ($bSlaveEnabled ? Phpfox::getPhrase('admincp.yes') : Phpfox::getPhrase('admincp.no')),
	    	'Total Mysql Replication Servers' => ($bSlaveEnabled ? count(Phpfox::getParam(array('db', 'slave_servers'))) : Phpfox::getPhrase('admincp.n_a')),
	    	// Phpfox::getPhrase('admincp.sql_slave_server') => ($bSlaveEnabled ? Phpfox::getLib('database')->sSlaveServer : Phpfox::getPhrase('admincp.n_a')),
	    	'Memory Usage (Limit)' => $oFile->filesize($this->_getUsableMemory()) . ' (' . (ini_get('memory_limit') == '-1' ? 'None' : @ini_get('memory_limit')) . ')',
