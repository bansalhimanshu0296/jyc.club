		...
		<div class="row_title_info">    
			...
			<div id="js_controller_music_play_{$aSong.song_id}">
				...
				<div class="music_title_link">
					<header><h1><a href="{permalink module='music' id=$aSong.song_id title=$aSong.title}" class="link" title="{$aSong.title|clean}" {if defined('PHPFOX_IS_POPUP')} onclick="window.opener.location.href=this.href; return false;"{/if}>{$aSong.title|clean|shorten:35:'...'|split:20}</a></h1></header>
				</div>			
				<div class="clear"></div>
			</div>
			...
			<div class="extra_info">
				<ul class="extra_info_middot"><li><a href="{permalink module='music.album' id=$aSong.album_id title=$aSong.album_name}" title="{$aSong.album_name|clean}">{$aSong.album_name|clean|shorten:55:'...'|split:40}</a></li><li>{$aSong.time_stamp|convert_time} {phrase var='music.by_lowercase'} {$aSong|user:'':'':50}</li>{if $aSong.total_play > 1}<li><span>&middot;</span></li><li>{phrase var='music.total_plays' total=$aSong.total_play}</li>{/if}</ul>
			</div>
			{/if}
