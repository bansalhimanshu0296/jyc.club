	...
	<div id="js_album_{$aAlbum.album_id}" class="js_album_parent {if $aAlbum.is_sponsor}row_sponsored {/if}{if $aAlbum.is_featured}row_featured {/if}{if is_int($phpfox.iteration.albums/2)}row1{else}row2{/if}{if $phpfox.iteration.albums == 1} row_first{/if}">
		<div class="row_title">
			...
			<div class="row_title_info">
				<header><h1><a href="{permalink module='music.album' id=$aAlbum.album_id title=$aAlbum.name}" class="link" title="{$aAlbum.name|clean}" {if defined('PHPFOX_IS_POPUP')} onclick="window.opener.location.href=this.href; return false;"{/if}>{$aAlbum.name|clean|shorten:50:'...'|split:30}</a></h1></header>
