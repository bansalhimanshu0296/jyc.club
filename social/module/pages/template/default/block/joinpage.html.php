...
{if !Phpfox::getUserBy('profile_page_id') && Phpfox::isUser()}
	...
	{else}
		...
		{else}
			{if isset($aPage)}
				{if $aPage.is_liked}
					<a href="#" id="pages_unlike_join" onclick="$(this).hide(); $.ajaxCall('like.delete', 'type_id=pages&amp;item_id={$aPage.page_id}'); return false;">
				...
				{else}
					<a href="#" id="pages_like_join" onclick="$(this).hide(); {if $aPage.page_type == '1' && $aPage.reg_method == '1'} $.ajaxCall('pages.signup', 'page_id={$aPage.page_id}'); {else}$.ajaxCall('like.add', 'type_id=pages&amp;item_id={$aPage.page_id}');{/if} return false;">
						{if $aPage.page_type == '1' }
