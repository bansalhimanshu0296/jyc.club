...
class Pages_Service_Process extends Phpfox_Service 
{
	...
	{		
		...
		{			
			...
			// Unity, update the menus so that they show the image properly in the left menu.
			if(Phpfox::getLib('template')->getStyleFolder() == 'unity')
			{
				$sCachedId = Phpfox::getLib('cache')->set(array('pages', Phpfox::getUserId()));
				Phpfox::getLib('cache')->remove($sCachedId);
			}
		}		
