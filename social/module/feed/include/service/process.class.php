...
{	
	...
	{			
		...
		$sFeedData = null;
		if (is_array($iItemId)) {
			unset($iItemId['user_id'], $iItemId['type']);
			$sFeedData = json_encode($iItemId);
			$iItemId = 0;
		}
	...
	public function update_stream($stream_id, $json_data) {
		$feed = $this->database()->select('*')
			->from(Phpfox::getT('feed'))
			->where('feed_id = ' . (int) $stream_id)
			->execute('getSlaveRow');
		...
		if (!empty($feed['feed_data'])) {
			$old_data = (array) json_decode($feed['feed_data'], true);
		}

		$this->database()->update(Phpfox::getT('feed'), array('feed_data' => json_encode(array_merge($old_data, $json_data))), 'feed_id = ' . (int) $feed['feed_id']);
