...
{	
	...
	public function get($iUserid = null, $iFeedId = null, $iPage = 0, $bForceReturn = false)
	{
		...
		if (Phpfox::getUserBy('feed_sort') || defined('PHPFOX_IS_USER_PROFILE') || defined('PHPFOX_IS_PAGES_VIEW'))
		{
		...
		{
			$pre_where = '';
			if (defined('PHPFOX_IS_APP_STREAM')) {
				$pre_where = 'feed.type_id = \'' . $this->database()->escape(PHPFOX_IS_APP_STREAM) . '\' AND ';
			}
			...
			if (Phpfox::isModule('privacy') && Phpfox::getUserParam('privacy.can_view_all_items'))
			{
				...
				$aRows = $this->database()->select($sSelect)
						...
						->group('feed.feed_id')
						->limit($iOffset, $iTotalFeeds)			
						->where($pre_where . 'feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
						->execute('getSlaveRows');
			}
			else
			{
				...
				{					
					if (Phpfox::isModule('friend'))
					{
						...
						$this->database()->select('feed.*')
							...
							->where($pre_where . 'feed.privacy IN(0,1,2) AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
							// ->limit($iTotalFeeds)
							->union();
						...
						$this->database()->select('feed.*')
							->from($this->_sTable, 'feed')
							->where($pre_where . 'feed.privacy IN(0,1,2,3,4) AND feed.user_id = ' . Phpfox::getUserId() . ' AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
							// ->limit($iTotalFeeds)
							->union();
					}
				}
				...
				{				
					...
					if (Phpfox::isModule('friend'))
					{
						...
						$this->database()->select('feed.*')
							->from($this->_sTable, 'feed')
							->join(Phpfox::getT('friend'), 'f', 'f.user_id = feed.user_id AND f.friend_user_id = ' . Phpfox::getUserId())
							->where($pre_where . 'feed.privacy IN(1,2) AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
							->limit($iTotalFeeds)
							->union();		
						...
						$this->database()->select('feed.*')
							...
							->where($pre_where . 'feed.privacy IN(2) AND feed.time_stamp > \'' . $iLastActiveTimeStamp .  '\' AND feed.feed_reference = 0')
							->limit($iTotalFeeds)
							->union();
					}				
					...
					$this->database()->select('feed.*')
						->from($this->_sTable, 'feed')
						->where($pre_where . 'feed.privacy IN(' . $sMyFeeds . ') AND feed.user_id = ' . Phpfox::getUserId() . ' AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
						->union();

					// Get public feeds
					$this->database()->select('feed.*')
						->from($this->_sTable, 'feed')
						->where($pre_where . 'feed.privacy IN(0) AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
						->union();					
					...
					$this->database()->select('feed.*')
						->from($this->_sTable, 'feed')						
						->where($pre_where . 'feed.privacy IN(4) AND feed.time_stamp > \'' . $iLastActiveTimeStamp . '\' AND feed.feed_reference = 0')
						->union();				
				}
	...
	*	but it also controls if the entire div should be shown or not, including the dislikes
	...
	public function getPhraseForLikes(&$aFeed, $bForce = false)
	{
		...
		if (!isset($aFeed['feed_is_liked'])) {
			$aFeed['feed_is_liked'] = $this->database()->select('COUNT(*)')->from(Phpfox::getT('like'))->where('type_id = \'' . $this->database()->escape($aFeed['type_id']) . '\' AND item_id = ' . (int) (isset($aFeed['feed_id']) ? $aFeed['feed_id'] : $aFeed['item_id']) . ' AND user_id = ' . Phpfox::getUserId())->execute('getSlaveField');;
		}
	    ...
	    if (isset($aFeed['likes']) && is_array($aFeed['likes']) && $iCountLikes > 0)
	    {
			...
			if ( isset($aFeed['feed_total_like']) && ($aFeed['feed_total_like'] > Phpfox::getParam('feed.total_likes_to_display')) && Phpfox::getParam('feed.total_likes_to_display') != 1)
			{
	...
	{			
		...
		if (!Phpfox::isApp($aRow['type_id']) && !Phpfox::hasCallback($aRow['type_id'], 'getActivityFeed'))
		{
			return false;
		}
		...
		{
			if (Phpfox::isApp($aRow['type_id'])) {
				if (empty($aRow['feed_data'])) {
					return false;
				}
				$aFeedData = json_decode($aRow['feed_data'], true);
				// $iIsLiked = $this->database()->select('COUNT(*)')->from(Phpfox::getT('like'))->where('type_id = \'' . $this->database()->escape($aRow['type_id']) . '\' AND item_id = ' . (int) $aRow['feed_id'] . ' AND user_id = ' . Phpfox::getUserId())->execute('getSlaveField');
				$aRow['item_id'] = $aRow['feed_id'];

				$output = $aFeedData;
				unset($output['title'], $output['content'], $output['user_id'], $output['type']);

				$custom = array(
					'feed_link' => Phpfox::getLib('url')->permalink($aRow['type_id'], $aRow['feed_id'], (empty($aFeedData['title']) ? '' : $aFeedData['title'])),
					'enable_like' => true,
					'comment_type_id' => $aRow['type_id'],
					'like_type_id' => $aRow['type_id'],
					'like_item_id' => $aRow['feed_id'],
					// 'feed_is_liked' => ($iIsLiked ? true : false),
					'total_comment' => $aRow['total_comment'],
					'feed_total_like' => $aRow['total_boost'],
					'feed_data' => $aFeedData,
					'custom_output' => json_encode($output)
				);

				if (!empty($aFeedData['title'])) {
					$custom['feed_title'] = $aFeedData['title'];
					$custom['feed_content'] = $aFeedData['content'];
				} else {
					$custom['feed_status'] = $aFeedData['content'];
				}

				$aFeed = array_merge($aRow, $custom);
			...
			{
				$aFeed['likes'] = Phpfox::getService('like')->getLikesForFeed($aFeed['like_type_id'], (isset($aFeed['like_item_id']) ? $aFeed['like_item_id'] : $aRow['item_id']), false, Phpfox::getParam('feed.total_likes_to_display'), true);
