...
class Admincp_Component_Controller_Setting_Edit extends Phpfox_Component 
{
	...
	{	
		...
		if ($this->request()->get('module-id')) {
			$this->template()->setBreadCrumb('Modules', '#module')
				->setBreadCrumb(Phpfox::getPhraseT($sSettingTitle, 'module'), '#' . $this->request()->get('module-id'))
				->setBreadCrumb('Settings', $this->url()->makeUrl('current'), true);
		} else {
			$this->template()->setBreadCrumb('Settings', '#')->setBreadCrumb(Phpfox::getPhraseT($sSettingTitle, 'module'), null, true);
		}
