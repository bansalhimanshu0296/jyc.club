...
class Admincp_Component_Controller_Product_Index extends Phpfox_Component 
{
	...
	public function process()
	{
		...
		if ($aVals = $this->request()->getArray('val'))
		{
			...
			{
				$this->url()->send('admincp', null, Phpfox::getPhrase('admincp.product_s_updated'));
			}			
		}				
		...
		if ($sDeleteProduct = $this->request()->get('delete'))
		{
			...
			{
				$this->url()->send('admincp', null, Phpfox::getPhrase('admincp.product_successfully_deleted'));
			}
		}
		...
		if (($sUpgrade = $this->request()->get('upgrade')))
		{
			...
			if (Phpfox::getService('admincp.product.process')->upgrade($sUpgrade))
			{
				...
				$this->url()->send('admincp', null, Phpfox::getPhrase('admincp.product_successfully_upgraded'));
			}
		}
