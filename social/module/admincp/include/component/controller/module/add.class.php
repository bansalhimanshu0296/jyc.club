...
class Admincp_Component_Controller_Module_Add extends Phpfox_Component 
{
	...
	public function process()
	{
		...
		if ($this->request()->get('active') || $this->request()->get('active') === '0') {
			if (Phpfox::getService('admincp.module.process')->updateActivity($this->request()->get('id'), $this->request()->get('active'))) {
				$this->url()->send('admincp', null, 'Module successfully updated.');
			}
		}
