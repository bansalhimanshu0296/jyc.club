...
class Admincp_Service_Module_Module extends Phpfox_Service  
{
	...
	public function getAdminMenu()
	{
		...
		{	
			...
			{
				if ($aRow['module_id'] == 'core'
					|| $aRow['module_id'] == 'custom'
					|| $aRow['module_id'] == 'theme'
					|| $aRow['module_id'] == 'track'
					|| $aRow['module_id'] == 'search'
					|| $aRow['module_id'] == 'language'
				) {
					unset($aRows[$iKey]);
				...
				$total_settings = $this->database()->select('COUNT(*)')->from(Phpfox::getT('setting'))->where('module_id = \'' . $aRow['module_id'] . '\' AND is_hidden = 0')->execute('getSlaveField');
				$total_user_settings = $this->database()->select('COUNT(*)')->from(Phpfox::getT('user_group_setting'))->where('module_id = \'' . $aRow['module_id'] . '\' AND is_hidden = 0')->execute('getSlaveField');
				if (!defined('MOXI9_IS_DEV') && !$aRow['is_menu'] && !$total_settings && !$total_user_settings)
				{
					unset($aRows[$iKey]);
				...
				if ($total_settings || $total_user_settings) {
					if (!isset($aRows[$iKey]['menu'])) {
						$aRows[$iKey]['menu'] = array();
					}

					$aRows[$iKey]['menu']['SPACE'] = true;
				}

				if ($total_settings) {
					$aRows[$iKey]['menu']['admincp.settings'] = array(
						'url' => 'setting.edit.module-id_' . $aRow['module_id']
					);
				}

				if ($total_user_settings) {
					$aRows[$iKey]['menu']['admincp.user_group_settings'] = array(
						'url' => 'user.group.settings.id_' . $aRow['module_id']
					);
				}

				if (!defined('MOXI9_IS_DEV') && !file_exists(PHPFOX_DIR_MODULE . $aRow['module_id'] . PHPFOX_DS . 'include' . PHPFOX_DS . 'component' . PHPFOX_DS . 'controller' . PHPFOX_DS . 'admincp' . PHPFOX_DS . 'index.class.php')) {
					$aRows[$iKey]['no_index'] = 'admincp.setting.edit.module-id_' . $aRow['module_id'];
				}

				if (defined('MOXI9_IS_DEV')) {
					$aRows[$iKey]['no_index'] = 'admincp.module.add.id_' . $aRow['module_id'];
				}

				if (!$aRow['is_active']) {
					$aRows[$iKey]['no_index'] = 'admincp.module.enable.id_' . $aRow['module_id'];
				}
				
				$aRows[$iKey]['url_name'] = $oFilter->cleanTitle($aRow['module_id']);
				$aRows[$iKey]['app_name'] = (empty($aRow['app_name']) ? $aRow['module_id'] : $aRow['app_name']);
