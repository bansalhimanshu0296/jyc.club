...
class Admincp_Service_Menu_Process extends Phpfox_Service 
{
	...
	public function isActive($id, $active) {
		$this->database()->update($this->_sTable, array('is_active' => (int) $active), 'menu_id = ' . (int) $id);
	...
	{
		$ordering = 0;
		foreach (explode(',', $aVals) as $iId)
		{
			$ordering++;

			$this->database()->update($this->_sTable, array('ordering' => (int) $ordering), 'menu_id = ' . (int) $iId);
		}
