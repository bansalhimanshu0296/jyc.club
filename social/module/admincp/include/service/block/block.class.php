...
class Admincp_Service_Block_Block extends Phpfox_Service 
{
	...
	public function getList($connection, $location) {
		$blocks = $this->database()->select('b.*, m.module_id AS module_name')
			->from($this->_sTable, 'b')
			->where('m_connection = \'' . $connection . '\' AND location = ' . (int) $location)
		...
		foreach ($blocks as $key => $block) {
			if (empty($block['title'])) {
				$block['title'] = $block['module_id'] . '.' . $block['component'];
			}
			$blocks[$key]['title'] = Phpfox::getLib('locale')->convert($block['title']);
		}
