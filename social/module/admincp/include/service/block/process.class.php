...
class Admincp_Service_Block_Process extends Phpfox_Service 
{
	...
	public function processCustomBlock($val) {

		if (!empty($val['edit_id'])) {

			$this->database()->update($this->_sTable, array(
				'title' => $val['title']
			), 'block_id = ' . (int) $val['edit_id']);

			$this->database()->update(Phpfox::getT('block_source'), array(
				'source_code' => $val['html'],
				'source_parsed' => $val['html']
			), 'block_id = ' . (int) $val['edit_id']);
		...
		$block_id = $this->database()->insert($this->_sTable, array(
			'title' => $val['title'],
			'm_connection' => $val['controller'],
			'module_id' => 'core',
			'type_id' => 2,
			'product_id' => 'phpfox',
			'component' => null,
	...
	public function processOrdering($blocks) {

		$new_id = 0;
		foreach ($blocks as $block) {
			list($location, $block_id, $ordering) = explode(':', $block);

			if (substr($block_id, 0, 10) == 'NEW_BLOCK_') {
				list($module, $component, $connection) = explode(';', substr_replace($block_id, '', 0, 10));

				$check = $this->database()->select('block_id')->from($this->_sTable)->where('
					m_connection = \'' . $this->database()->escape($connection) . '\'
					AND module_id = \'' . $this->database()->escape($module) . '\'
					AND component = \'' . $this->database()->escape($component) . '\'
				')->execute('getRow');

				if (isset($check['block_id'])) {
					$block_id = $check['block_id'];
				} else {
					$new_id = $this->database()->insert($this->_sTable, array(
			...
			$this->database()->update($this->_sTable, array('location' => $location, 'ordering' => $ordering), 'block_id = ' . (int) $block_id);
		}
	...
	public function updateName($block_id, $name) {
		$this->database()->update($this->_sTable, array('title' => $name), 'block_id = ' . (int) $block_id);
		$this->cache()->remove('block', 'substr');
	}
