...
class Announcement_Component_Controller_Admincp_Add extends Phpfox_Component
{
    public function process()
    {
	    ...
	    $this->template()->setBreadcrumb('Modules', '#modules')
	...
	if ($iEdit = $this->request()->getInt('id'))
	{
		...
		$this->template()->assign(array(
		    ...
		    ->setBreadcrumb(Phpfox::getPhrase('announcement.edit_an_announcement'), $this->url()->makeUrl('current'), true);
	...
	{
	    $this->template()->setBreadcrumb('New Announcement', $this->url()->makeUrl('current'), true);
	    $aAnnouncement = array();
