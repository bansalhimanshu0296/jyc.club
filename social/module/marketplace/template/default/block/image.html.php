...
{/if}
	<div class="marketplace_image_holder">		
		<div class="marketplace_image">
			<a class="js_listing_click no_ajax_link" href="#" data-href="{img return_url=true server_id=$aListing.server_id title=$aListing.title path='marketplace.url_image' file=$aListing.image_path suffix='_400'}">
		...
		<div class="marketplace_image_extra js_box_image_holder_thumbs">
			<ul>{foreach from=$aImages name=images item=aImage}<li><a class="js_listing_click no_ajax_link" href="#" data-href="{img return_url=true server_id=$aImage.server_id title=$aListing.title path='marketplace.url_image' file=$aImage.image_path suffix='_400'}">{img server_id=$aImage.server_id title=$aListing.title path='marketplace.url_image' file=$aImage.image_path suffix='_50_square' max_width='50' max_height='50'}</a></li>{/foreach}</ul>
			<div class="clear"></div>
		</div>	
		{/if}
	</div>
