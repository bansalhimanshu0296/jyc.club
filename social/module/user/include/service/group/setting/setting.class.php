...
class User_Service_Group_Setting_Setting extends Phpfox_Service 
{
	...
	public function getByModule($module_id) {
		$settings = array();
		$user_groups = $this->database()->select('*')->from(Phpfox::getT('user_group'))->execute('getSlaveRows');

		$settings[] = array(
			'group' => false,
			'settings' => $this->get(ADMIN_USER_ID, $module_id)
		);
		foreach ($user_groups as $user_group) {
			$settings[] = array(
				'group' => $user_group,
				'settings' => $this->get($user_group['user_group_id'], $module_id)
			);
		}
