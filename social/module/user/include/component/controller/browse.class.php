...
class User_Component_Controller_Browse extends Phpfox_Component
{
    ...
    {    	
		...
		$aPages = (Phpfox::isMobile() ? array(10) : array(20, 30, 40, 50));
		$aDisplays = array();
	    ...
	    if (defined('PHPFOX_IS_ADMIN_SEARCH')) {
		    $this->template()->setTitle(Phpfox::getPhrase('user.browse_members'))
			    ->setBreadcrumb('Members', '#members')
			    ->setBreadCrumb('Manage', $this->url()->makeUrl('admincp.user.browse'), true);
	    } else {
		...
		*/
		...
		if (!(defined('PHPFOX_IS_ADMIN_SEARCH')))
		{
			...
			Phpfox::getLib('pager')->set(array('page' => $iPage, 'size' => $iPageSize, 'count' => $iCnt, 'aParams' => $aNewCustomValues));
		}
		else
		{
