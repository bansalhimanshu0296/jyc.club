...
class User_Component_Controller_Admincp_Cancellations_Add extends Phpfox_Component
{
	...
	public function process()
	{
		...
		Phpfox::getService('user.cancellations')->menu($this->template());
		
		$this->template()->setTitle(Phpfox::getPhrase('user.add_cancellation_options'))
			->setBreadCrumb('Members', '#members')
			->setBreadcrumb('Cancellation Options', $this->url()->makeUrl('admincp.user.cancellations.manage'))
			->setBreadcrumb('Create New Option', $this->url()->makeUrl('admincp.user.cancellations.add'), true)
			->assign(array(
					// used when editing
				)
			);		
