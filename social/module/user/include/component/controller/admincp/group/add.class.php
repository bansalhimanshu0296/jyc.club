...
class User_Component_Controller_Admincp_Group_Add extends Phpfox_Component 
{
	...
	{			
		...
		{		
			...
			->setBreadcrumb('User Groups', $this->url()->makeUrl('admincp.user.group'))
			->setBreadcrumb('Settings: ' . Phpfox::getLib('locale')->convert($aGroup['title']) . ' (ID#' . $aGroup['user_group_id'] . ')', $this->url()->makeUrl('current'), true)
