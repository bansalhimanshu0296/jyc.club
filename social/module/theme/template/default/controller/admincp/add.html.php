...
<form method="post" action="{url link='admincp.theme.add'}">
	...
	<div><input type="hidden" name="val[parent_id]" value="{$default_theme}" /></div>
	<div class="table">
	...
	<div class="table" style="display:none;">
		...
		<div class="table_right">
			<select name="val[total_column]" id="total_column">
				<option value="1">{phrase var='theme.select'}</option>
	...
	<div class="table_clear">
		...
		{if $bIsEdit && defined('MOXI9_IS_DEV') && $aForms.folder != 'unity'}
		<a href="#" style="position:absolute; right:10px; top:10px;" onclick="if (confirm('Are you sure?')) {l} $.ajaxCall('theme.deleteTheme', '&id={$aForms.theme_id}'); {r} return false;">Delete this theme</a>
		{/if}
	</div>
</form>
