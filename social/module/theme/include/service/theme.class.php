...
{	
	...
	public function unity() {
		$rows = $this->database()->select('t.*, COUNT(ts.theme_id) AS total_style')
			->from($this->_sTable, 't')
		...
		foreach ($rows as $row) {
			...
			if ($row['folder'] == 'unity') {
				$row['name'] = 'Default';
				$row['image'] = 'theme/frontend/default/style/default/image/layout/9c015e23637969877387b93dbce60888.jpg';
			} else {
				$data = $row['website'];
				if (substr($data, 0, 1) == '{') {
					$data = json_decode($data);
					$row['image'] = 'http://nullrefer.com/?http://d2h79mkp7etn4r.cloudfront.net/icon/' . $data->i;
				}
			}
	...
	{
		$aTheme = $this->database()->select('*')
			->from($this->_sTable)
			->where(($bUseFolder ? 'folder = \'' . $this->database()->escape($iId) . '\'' : "theme_id = " . (int) $iId))
			->execute('getRow');

		$aTheme['default_style'] = $this->database()->select('*')
			->from(Phpfox::getT('theme_style'))
			->where('theme_id = ' . (int) $aTheme['theme_id'] . ' AND is_active = 1')
			->order('style_id ASC')
			->execute('getRow');
