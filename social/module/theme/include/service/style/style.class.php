...
class Theme_Service_Style_Style extends Phpfox_Service 
{
	...
	public function getImageDir($style_id) {
		$style = $this->getStyle($style_id);
		$path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'image' . PHPFOX_DS;
		...
		if (is_dir($path)) {
			foreach (scandir($path) as $file) {
				...
				$images[] = array(
					'url' => str_replace(PHPFOX_DIR, Phpfox::getParam('core.path'), $path) . $file,
					'name' => $file
				);
			}
		}
	...
	public function getStyleContent($style_id)
	{
		$style = $this->getStyle($style_id);

		$content = '';
		$path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'css' . PHPFOX_DS . 'custom.css';
		$custom_path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'css' . PHPFOX_DS . 'custom-build.css';
		if (file_exists($custom_path)) {
			$content = file_get_contents($custom_path);
		}
	...
	public function getJs($style_id) {
		$style = $this->getStyle($style_id);

		$content = '';
		$path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'jscript' . PHPFOX_DS . 'custom.js';
		$custom_path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'jscript' . PHPFOX_DS . 'custom-build.js';
		if (file_exists($custom_path)) {
			$content = file_get_contents($custom_path);
		}
		else if (file_exists($path)) {
