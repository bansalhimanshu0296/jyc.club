...
class Theme_Service_Style_Process extends Phpfox_Service 
{
	...
	public function updateJs($aVals) {
		Phpfox::isUser(true);
		Phpfox::getUserParam('admincp.has_admin_access', true);

		$theme = Phpfox::getService('theme')->getTheme($aVals['theme_id']);
		$style = Phpfox::getService('theme.style')->getStyle($theme['default_style']['style_id']);

		$path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $style['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $style['folder'] . '' . PHPFOX_DS . 'jscript' . PHPFOX_DS . 'custom-build.js';
		file_put_contents($path, $aVals['css_data']);
		/*
		$this->database()->insert(Phpfox::getT('theme_js'), array(
			'theme_id' => $aVals['theme_id'],
			'js_content' => $aVals['css_data'],
	...
	public function update($aVals)
	{
		...
		$path = PHPFOX_DIR . 'theme' . PHPFOX_DS . 'frontend' . PHPFOX_DS . '' . $aStyle['theme_folder'] . '' . PHPFOX_DS . 'style' . PHPFOX_DS . '' . $aStyle['folder'] . '' . PHPFOX_DS . 'css' . PHPFOX_DS . 'custom-build.css';
		file_put_contents($path, $aVals['css_data']);
