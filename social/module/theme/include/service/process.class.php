...
class Theme_Service_Process extends Phpfox_Service 
{
	...
	public function add($aVals, $iEditId = null, $sXmlData = null, $bIsImport = false)
	{
		...
		if ($iEditId === null)
		{
			...
			mkdir($sDir);
			mkdir($sDir . 'template' . PHPFOX_DS);
			mkdir($sDir . 'style' . PHPFOX_DS);
			mkdir($sDir . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS);
			mkdir($sDir . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'css' . PHPFOX_DS);
			mkdir($sDir . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'image' . PHPFOX_DS);
			mkdir($sDir . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'jscript' . PHPFOX_DS);

			$style = $this->database()->select('style_id')
				->from(Phpfox::getT('theme_style'))
				->where('theme_id = ' . (int) $aVals['parent_id'])
				->execute('getRow');

			$this->database()->insert(Phpfox::getT('theme_style'), array(
				'theme_id' => $iId,
	...
	public function setDefault($theme_id) {
		$this->database()->update(Phpfox::getT('theme') , array('is_default' => '0'), 'theme_id > 0');
		$this->database()->update(Phpfox::getT('theme_style') , array('is_default' => '0'), 'style_id > 0');

		$this->database()->update(Phpfox::getT('theme') , array('is_default' => '1'), 'theme_id = ' . (int) $theme_id);
		$this->database()->update(Phpfox::getT('theme_style') , array('is_default' => '1'), 'theme_id = ' . (int) $theme_id);
	...
	public function unityInstall($data) {

		$folder = $data->product->folder;
		$path = PHPFOX_DIR_THEME . 'frontend' . PHPFOX_DS . $folder . PHPFOX_DS;
		if (!is_dir($path)) {
			mkdir($path);
			mkdir($path . 'template' . PHPFOX_DS);
			mkdir($path . 'style' . PHPFOX_DS);
			mkdir($path . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS);
			mkdir($path . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'css' . PHPFOX_DS);
			mkdir($path . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'image' . PHPFOX_DS);
			mkdir($path . 'style' . PHPFOX_DS . 'default' . PHPFOX_DS . 'jscript' . PHPFOX_DS);
		}

		$check = $this->database()
			->select('*')
			->from(Phpfox::getT('theme'))
			->where('folder = \'' . $this->database()->escape($folder) . '\'')
			->execute('getRow');

		if (!isset($check['theme_id'])) {
			$parent = $this->database()->select('*')
				->from(Phpfox::getT('theme'))
				->where('folder = \'unity\'')
				->execute('getRow');

			$style_parent = $this->database()->select('*')
				->from(Phpfox::getT('theme_style'))
				->where('folder = \'unity\'')
				->execute('getRow');

			$theme_id = $this->database()->insert(Phpfox::getT('theme'), array(
				'parent_id' => $parent['theme_id'],
				'name' => $data->product->name,
				'folder' => $folder,
				'created' => PHPFOX_TIME,
				'website' => '{"l":' . $data->product->id . ',"i": "' . $data->product->icon . '"}',
				'is_active' => '1'
			));

			$this->database()->insert(Phpfox::getT('theme_style'), array(
				'theme_id' => $theme_id,
				'parent_id' => $style_parent['theme_id'],
				'is_active' => '1',
				'name' => $data->product->name,
				'folder' => 'default',
				'created' => PHPFOX_TIME
		...
		foreach ($data->files as $file => $content) {
			if (substr($file, 0, 6) == 'theme/') {
				file_put_contents(PHPFOX_DIR . $file, $content);

				continue;
			}

			if (substr($file, 0, 7) == 'module/') {
				$parts = explode('module/', $file);
				list($module, $file_extra) = explode('template/' . $folder . '/', $parts[1]);

				$type_id = 'block';
				if (substr($file_extra, 0, 11) == 'controller/') {
					$type_id = 'controller';
				}

				$sub_parts = explode($type_id . '/', $file_extra);
				$name = $sub_parts[1];
