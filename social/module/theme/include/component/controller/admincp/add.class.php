...
class Theme_Component_Controller_Admincp_Add extends Phpfox_Component
{
	...
	public function process()
	{
		...
		$bIsEdit = false;
		$aThemes = Phpfox::getService('theme')->unity();
		...
		if (($aVals = $this->request()->getArray('val')))
		{
			...
			{
				if (($id = Phpfox::getService('theme.process')->add($aVals)))
				{
					$this->url()->send('admincp.theme.manage', array('id' => $id), Phpfox::getPhrase('theme.theme_successfully_added'));
				}
			}
		}
		...
		foreach ($aThemes as $theme) {
			if ($theme['folder'] == 'unity') {
				$default_theme = $theme['theme_id'];
				break;
			}
		}
