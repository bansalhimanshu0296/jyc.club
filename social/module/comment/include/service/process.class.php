...
class Comment_Service_Process extends Phpfox_Service
{
	...
	public function add($aVals, $iUserId = null, $sUserName = null)
	{
		...
		if (!Phpfox::isApp($aVals['type'])) {
			...
			$feed = $this->database()->select('user_id')->from(Phpfox::getT('feed'))->where('feed_id = ' . (int) $aVals['item_id'])->execute('getSlaveRow');
			$aItem = array(
				'comment_view_id' => 0,
				'comment_user_id' => $feed['user_id']
			);
		}
		...
		$aInsert = array(
			...
			'view_id' => (isset($aItem) ? (($aItem['comment_view_id'] == 2 && $aItem['comment_user_id'] != $iUserId) ? '1' : '0') : 0),
		...
		if (Phpfox::isApp($aVals['type'])) {
			$this->database()->updateCounter('feed', 'total_comment', 'feed_id', (int) $aVals['item_id']);
		}
		...
		if (isset($aItem) && ($aItem['comment_view_id'] == 2 && $aItem['comment_user_id'] != $iUserId))
		{
