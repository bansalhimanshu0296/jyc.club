...
class Event_Component_Controller_Admincp_Add extends Phpfox_Component
{
	...
	public function process()
	{
		...
		$this->template()->setTitle(($bIsEdit ? Phpfox::getPhrase('event.edit_a_category') : Phpfox::getPhrase('event.create_a_new_category')))
			...
			->setBreadcrumb('Events', $this->url()->makeUrl('admincp.event'))
			->setBreadcrumb(($bIsEdit ? Phpfox::getPhrase('event.edit_a_category') : Phpfox::getPhrase('event.create_a_new_category')), $this->url()->makeUrl('current'), true)
			->assign(array(
