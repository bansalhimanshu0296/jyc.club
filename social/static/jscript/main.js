...
$.ajaxProcess = function(sMessage, sSize)
{
	...
	return '<span style="margin-left:4px; margin-right:4px; font-size:9pt; font-weight:normal;"><img src="' + eval('oJsImages.ajax_' + sSize + '') + '" class="v_middle" /></span>';
};
...
$Behavior.ajax_pager = function() {
	if ($('.pager_link').length) {
		$('.pager_link').each(function() {
			var this_object = $('.pager_link'),
				pager_parent = this_object.parents('.pager_link_holder:first'),
			    pager_function = function(obj) {
				$.ajax({
					url: obj.html().replace('<span>', '').replace('</span>', ''),
					data: 'is_ajax_pager=true&is_ajax=true',
					complete: function(data) {
						pager_parent.find('.pager_link_append').append(data.responseText);
						$Core.loadInit();
					}
				});
			};

			if ($Core.isInView(this_object)) {
				pager_function(this_object);
				this_object.remove();
			}
...
$Behavior.globalToolTip = function()
{
	...
	$('.js_hover_title').mouseover(function()
	{
		...
		else
		{
			...
			var this_top = (offset.top - ($('#js_global_tooltip_display').height() + 10));
			if (this_top <= 0) {
				this_top = (offset.top + ($('#js_global_tooltip_display').height() + 10))
			}
			$('#js_global_tooltip').css('top', this_top + 'px');
		}		
	});
...
$Core.is_mobile = function() {
	return ($(window).width() <= 1100 ? true : false);
};

$Behavior.globalInit = function()
{
	var is_mobile = $Core.is_mobile();

	$('.notify_drop_link').click(function() {
		...
		$('.notify_drop_link.active').removeClass('active');
		$(this).addClass('active');
		if (!$('#panel_content').length) {
			$('body').prepend('<div id="panel_content"><div class="panel_title"></div><div class="panel_content"></div></div>');

			if (is_mobile) {
				$('#panel_content').height($(window).height() - 40).show();
			} else {
				$('#panel_content').css({
					'margin-right': '-300px',
		...
		$('#panel_content .panel_title').html($(this).attr('title'));
		$('#panel_content .panel_content').html('');
		$('.panel_content').css({
			height: $(window).height() - $('#panel_content .panel_title').innerHeight()
		})
		...
		if ($(this).attr('rel') == 'search') {
			var html_form = '';
			html_form += '<form method="post" action="' + $(this).attr('href') + '" id="js_temp_search">' +
				'<input type="text" name="q" placeholder="Search..." id="header_sub_menu_search_input" autocomplete="off" class="js_temp_friend_search_input" />' +
				'</form>'
			$('#panel_content .panel_title').html(html_form);
			$Core.loadInit();
		...
		$Core.ajax($(this).attr('rel'), {
			...
			success: function(data) {
				$('#panel_content .panel_content').html(data);
				$Core.loadInit();
			}
		});
	...
	$('#js_temp_search').submit(function() {
		var search_for = $('#header_sub_menu_search_input').val();
		$.ajax({
			url: $(this).attr('action'),
			type: 'POST',
			data: 'is_ajax_post=true&q=' + search_for,
			complete: function(e) {
				if (e.responseText.substr(0, 1) == '[') {
					var html = '<ul class="search_results">';
					$.each($.parseJSON(e.responseText), function(key, result) {
						html += '<li><a href="' + result.item_link + '">' +
							result.item_title.replace(search_for, '<em>' + search_for + '</em>') +
							'<div class="sr_type">' + result.item_name + '</div>' +
							'</a></li>';
					});
					html += '</ul>';
	...
	if ($('.item_is_active').length) {
		$('.item_is_active, .item_is_not_active').each(function() {
			if ($(this).find('input').attr('checked')) {
				if ($(this).hasClass('item_is_active')) {
					$(this).parent().find('.item_is_not_active').addClass('is_off');
				} else {
					$(this).parent().find('.item_is_active').addClass('is_off');
				}
			}
		});
	}
    ...
    $('.js_item_active').click(function()
    {
	    ...
	    $(this).parent().find('.is_off').removeClass('is_off');
    	if ($(this).hasClass('item_is_active'))
    	{
	...
	{    
    	...
    	{    		
    		...
    		if ($(this).hasClass('is_clicked'))
    		{
			    ...
			    $('.moderation_holder ul').removeClass('active');
    			$(this).removeClass('is_clicked');
    		}
    		else
    		{
    			// $('.moderation_holder ul').show();
			    $('.moderation_holder ul').addClass('active');
...
$Core.getRequests = function(sHref, bReturnPath)
{
	...
	switch (oCore['core.url_rewrite'])
	{
		case '1':
			...
			sUrlString = '/' + (aMatches === null ? '' : aMatches[1]);
			break;
		case '3':
...
$Core.guest_panel_drop = function() {
	var window_height = $(window).height();

	$('.guest_panel_drop').show();
	var panel_width = $('.guest_panel_drop').width() + 'px';
	$('.guest_panel_drop').hide();

	$('#guest_panel ul li a').click(function() {
		if ($('#guest_panel').hasClass('active') && !$(this).hasClass('active')) {
			//	$('.guest_panel_drop').fadeOut();
			var data_type = $(this).data('type');

			$('#guest_panel ul li a.active').removeClass('active');
			$(this).addClass('active');
			$('.guest_panel_drop:not(.gpd_' + $(this).data('type') + '').animate({
				'margin-right': '-=' + panel_width + ''
			}, 'fast', function() {
				$(this).hide();
				$('.gpd_' + data_type).css('margin-right', '-' + panel_width + '').height(window_height).show().animate({
					'margin-right': '+=' + panel_width + ''
				}, 'fast');
			});
		...
		if ($('#guest_panel').hasClass('active')) {
			$('#guest_panel').removeClass('active');
			$(this).removeClass('active');

			$('.gpd_' + $(this).data('type')).animate({
				'margin-right': '-=' + panel_width + ''
			}, 'fast', function() {
				$(this).hide();
			});

			$('#welcome_banner, #guest_panel').animate({
		...
		$('#guest_panel').addClass('active');
		$(this).addClass('active');
		$('.gpd_' + $(this).data('type')).css('margin-right', '-' + panel_width + '').height(window_height).show().animate({
			'margin-right': '+=' + panel_width + ''
		}, 'fast');
		$('#welcome_banner, #guest_panel').animate({
			'margin-left': '-=' + panel_width + ''
	...
	$('.gpd_close').click(function() {
		$('#guest_panel ul li a.active').click();
	});
};
...
$Core.ajaxMessage = function(message)
{
	$('#global_ajax_message').html((message ? message : getPhrase('core.saving'))).show().animate({
		top: '0px'
	});
};

$Core.actionBar = function() {
	if ($('.item_bar').length && !$Core.is_mobile() && !$('#photo_view_theater_mode').length) {
		var do_scroll = function(obj) {
			var lastScrollTop = 0,
				check_cnt = 0,
				is_down = false;
			$(window).scroll(function() {
				var st = $(this).scrollTop(),
					item_bar_height = obj.innerHeight();

				if (st > lastScrollTop) {
					check_cnt++;
					if (check_cnt === 5) {
						obj.animate({
							'margin-bottom': '-=' + item_bar_height + 'px'
						});
						is_down = true;
					}
				} else {
		...
		$('.item_bar_action').remove();
		$('.item_bar').css({
			position: 'fixed',
			bottom: '0px',
			left: $('.item_bar').offset().left + 'px',
			width: $('.item_bar').width()
		});
		do_scroll($('.item_bar'));
	}
};
...
$Core.page = function($aParams)
{
	...
	$('body').attr('id', $aParams['page_id']);
	$('body').attr('class', $aParams['class_name']);
	/*
...
{	
	...
	$('a').click(function()
	{
		...
		if ($(this).parents('ul:first').hasClass('links')) {
			$('.menu_is_selected').removeClass('menu_is_selected');
		...
		if ($('.notify_drop_link').hasClass('active')) {
			$('.notify_drop_link').removeClass('active');
			$('#panel_content').animate({
				'margin-right': '-=340px'
			}, 'slow', function() {
				$('#panel_content').remove();
		...
		var all_requests = $Core.getRequests(this),
			do_requests = $Core.getRequests(this, true);
		if (oCore['core.url_rewrite']) {
			var all_parts = all_requests.split('?');
			all_requests = all_parts[0];
			$.each(all_parts, function(key, value) {
				if (key === 0) {
					return;
				}
				...
				all_requests += '&' + sub_parts[0] + '=' + (isset(sub_parts[1]) ? sub_parts[1] : '');
			});

			var do_parts = do_requests.split('?');
			do_requests = do_parts[0];
		}

		$.ajaxCall('core.page', 'ajax_page_display=true' + all_requests + '&do=' + do_requests, 'GET');
...
(function() {
	...
	Core.action.unity_actions = function() {
		$('.ajax_post').submit(function() {
			var form_object = $(this);

			form_object.find('.tmp_errors').remove();
			form_object.find('.button').addClass('button_off').attr('disabled', true);
			form_object.find('.table_clear_ajax').show();

			Core.post($(this).attr('action'), $(this), function(data) {
				if (data.responseText.substr(0, 1) == '{') {
					...
					else if (isset(json_data.errors)) {
						form_object.prepend('<div class="tmp_errors"></div>');
						$.each(json_data.errors, function(key, error) {
							form_object.find('.tmp_errors').append('<div class="error_message">' + error + '</div>');
						});
					}
				} else {
				...
				form_object.find('.button').removeClass('button_off').attr('disabled', false);
				form_object.find('.table_clear_ajax').hide();
			});
	...
	Core.action.ace_editor = function() {
		if ($('#aceeditor').length) {
			$.getScript('http://cdnjs.cloudflare.com/ajax/libs/ace/1.1.3/ace.js', function() {
				$('#aceeditor').height($(window).height() - $('.form_panel_holder .table_clear').innerHeight() - 50).width($(window).width() - $('#top').width() - $('#form_panel').innerWidth() - 25);

				ace_editor = ace.edit('aceeditor');
				ace_editor.setTheme('ace/theme/xcode');
				ace_editor.getSession().setMode('ace/mode/' + ($(this).data('mode') ? $(this).data('mode') : 'php'));
			});
		}
	};
	...
	Core.url = function(app_id, path) {
		var url = getParam('sJsHome');

		url += app_id + '/' + (typeof(path) == 'undefined' ? '' : path);

		return url;
	};

	Core.post = function(action, obj, complete) {
		var params = {
			type: 'POST',
			url: action,
			dataType: 'script',
			data: 'is_ajax=1&' + $(obj).getForm()
		};
	...
	Core.build_upload = function(id) {
		p('Building upload on: ' + id);
		...
		$(document).on('dragover', function (e) {
			...
			$(id).addClass('on_drag_over');
		});
		$(document).on('drop', function (e) {
			e.stopPropagation();
			e.preventDefault();

			$(id).removeClass('on_drag_over');
		});
		var obj = $(id);
		obj.on('drop', function(e) {
			...
			Core.upload(files, obj);
		});
	};

	Core.upload = function(files, obj) {
		for (var i = 0; i < files.length; i++) {
			var fd = new FormData();
			fd.append('file', files[i]);

			$('body').prepend('<div class="upload_item" id="js_img_cnt_' + i + '"><span class="progress"></span><span class="name">' + files[i].name + '</span></div>');

			Core.send(fd, obj, i);
		}
	};

	Core.send = function(formData, obj, i) {
		var uploadURL = obj.parent().data('upload-url');
			...
			xhr: function() {
				...
				if (xhrobj.upload) {
					xhrobj.upload.addEventListener('progress', function(event) {
						...
						var this_progress = $('#js_img_cnt_' + i);
						this_progress.find('span.progress').width(percent + '%');
						if (percent == 100) {
							setTimeout(function() {
