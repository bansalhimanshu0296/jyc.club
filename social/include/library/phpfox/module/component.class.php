...
class Phpfox_Component
{
	...
	protected function no_trial($redirect = false) {
		$is_trial = false;
		...
		if ($is_trial) {
			define('PHPFOX_EXIT_PRODUCT', true);
		}
	}

	protected function is_dev($redirect = false) {
		$is_dev = false;
		...
		if ($is_dev) {
			$this->template()->assign('custom_exit_message', 'This requires a developers version.');
			define('PHPFOX_EXIT_PRODUCT', true);
		}
	}
}
