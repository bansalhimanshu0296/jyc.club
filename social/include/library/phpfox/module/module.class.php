...
{	
	...
	public function getApps() {
		return (array) $this->_apps;
	}

	public function isApp($sApp) {
		if (isset($this->_apps[$sApp])) {
			return true;
		}
	...
	{	
		...
		{
			if ($this->isApp($this->_sModule)) {
				$this->_sModule = 'api';
				$this->_sController = 'page';
			} else {
...
*/
	...
	public function _cacheModules()
	{
		...
		$cache_id = $Cache->set('apps');
		if (!($this->_apps = $Cache->get($cache_id))) {
			$rows = Phpfox::getLib('database')->select('*')
			...
			$this->_apps = array();
			foreach ($rows as $row) {
				$this->_apps[$row['app_id']] = $row;
			}

			$Cache->save($cache_id, $this->_apps);
		}

		if (is_array($this->_apps) && count($this->_apps)) {
			foreach ($this->_apps as $key => $value) {
				if (!empty($value['data'])) {
					$this->_apps[$key]['data'] = json_decode(utf8_encode($value['data']));
				}
			}
		}
