...
{	
	...
	{
		$this->setHeader(array('boot.css' => 'style_css'));
...
// You are filtering out the controllers which should not load 'content' ajaxly, finding a way for pages.view/1/info and like that
			...
			$aJsVars = array(
				...
				'sHostedVersionId' => (defined('MOXI9_HOSTED_VERSION') ? MOXI9_HOSTED_VERSION : '')
			);	
					...
					{		
						...
						switch ($mValue)
						{
							case 'style_script':
								...
								if ($mKey == 'custom.js') {
									$custom_file = str_replace(Phpfox::getParam('core.path'), PHPFOX_DIR, $this->getStyle('jscript', 'custom-build.js'));
									if (file_exists($custom_file)) {
										$aCacheJs[] = str_replace(Phpfox::getParam('core.path'), '', $this->getStyle('jscript', 'custom-build.js'));

										continue;
									}

									$actual_file = str_replace(Phpfox::getParam('core.path'), PHPFOX_DIR, $this->getStyle('jscript', 'custom.js'));
							...
							case 'style_css':
								if ($mKey == 'custom.css') {
									$custom_file = str_replace(Phpfox::getParam('core.path'), PHPFOX_DIR, $this->getStyle('css', 'custom-build.css'));
									if (file_exists($custom_file)) {
										$aCacheCSS[] = str_replace(Phpfox::getParam('core.path'), '', $this->getStyle('css', 'custom-build.css'));

										continue;
									}

									if (!file_exists($custom_file)) {
										$custom_file = str_replace(Phpfox::getParam('core.path'), PHPFOX_DIR, $this->getStyle('css', 'custom.css'));

										if (file_exists($custom_file)) {
											$aCacheCSS[] = str_replace(Phpfox::getParam('core.path'), '', $this->getStyle('css', 'custom.css'));
								...
								if ($mKey == 'boot.css') {
									$custom_file = str_replace(Phpfox::getParam('core.path'), PHPFOX_DIR, $this->getStyle('css', 'boot.css'));
									if (file_exists($custom_file)) {
										$aCacheCSS[] = str_replace(Phpfox::getParam('core.path'), '', $this->getStyle('css', 'boot.css'));
		...
		*/
		...
		if (!PHPFOX_IS_AJAX_PAGE) {
			foreach (Phpfox::getLib('module')->getApps() as $app) {
				if (isset($app['data']->header)) {
					$app['data']->header = str_replace('.css', '.css?v=' . Phpfox::getCleanVersion(), $app['data']->header);

					$sData .= $app['data']->header;
				}

				if (isset($app['data']->footer)) {
					$this->_sFooter .= $app['data']->footer;
				}
			}

			if (Phpfox::isUser()) {
				$this->_sFooter .= '<div id="js_active_user" data-active-id="' . Phpfox::getUserId() . '" data-active-name="' . Phpfox::getUserBy('full_name') . '" data-active-image="' . Phpfox::getLib('image.helper')->display(array('user' => Phpfox::getUserBy(), 'suffix' => '_50_square', 'return_url' => true)) . '"></div>';
			}

			if (Phpfox::isAdmin()) {
				$this->_sFooter .= '<div id="is_admin"><a href="' . Phpfox::getLib('url')->makeUrl('admincp') . '">View AdminCP</a></div>';
			}
	...
	public function getMenu($sConnection = null)
	{
		...
		if ((!($aMenus = $oCache->get($sCachedId))) && is_bool($aMenus) && !$aMenus)
		{
			...
			$aParents = Phpfox::getLib('database')->select('m.menu_id, m.parent_id, m.m_connection, m.var_name, m.disallow_access, mo.module_id AS module, m.url_value AS url, mo.is_active AS module_is_active')
				...
				->leftJoin(Phpfox::getT('module'), 'mo', 'mo.module_id = m.module_id AND mo.is_active = 1')
				->leftJoin(Phpfox::getT('product'), 'p', 'm.product_id = p.product_id AND p.is_active = 1')
		...
		foreach ($aMenus as $iKey => $aMenu)
		{
			...
			$aMenus[$iKey]['class_name'] = 'menu_' . str_replace(array('-', '.'), '_', $aMenu['url']);
...
*/
		...
		if ($sConnection == 'main' && Phpfox::isUser()) {
			...
			foreach ($aPages as $aPage) {
				$aMenus[] = array(
					'menu_id' => 0,
					'module' => null,
					'var_name' => $aPage['title'],
					'icon' => Phpfox::getLib('image.helper')->display(array('server_id' => $aPage['profile_server_id'], 'path' => 'core.url_user', 'file' => $aPage['profile_user_image'], 'suffix' => '_50_square', 'return_url' => true)),
					'class_name' => 'test',
					'url' => $aPage['link']
				);
			}
	...
	public function getPageId() {
		return 'page_' . str_replace(array('-', '.', '/'), '_', Phpfox::getLib('module')->getFullControllerName()) . '';
	}

	public function getPageClass() {
		if (defined('PHPFOX_IN_BLOCK_MODE')) {
			return ' design_block_mode ';
		}

		$class = '' . (defined('PHPFOX_IS_USER_PROFILE') ? ' is_user_profile' : '') . (defined('PHPFOX_IS_PAGES_VIEW') ? ' is_pages_profile' : '') . (Phpfox::isUser() ? ' is_user' : ' is_guest') . '';
		if (defined('PHPFOX_IS_APP_STREAM')) {
			$class .= ' app_stream_' . PHPFOX_IS_APP_STREAM;
		}
