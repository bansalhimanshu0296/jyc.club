...
class Phpfox_Template_Cache extends Phpfox_Template
{
	...
	{		
		...
		$sContent = str_replace('<body>', '<body id="<?php echo Phpfox::getLib(\'template\')->getPageId(); ?>" class="<?php echo Phpfox::getLib(\'template\')->getPageClass(); ?>">', $sContent);
	...
	private function _parseFunction($sFunction, $sModifiers, $sArguments)
	{
		switch ($sFunction)
		{
			...
			case 'block':
				...
				$sContent .= '<div class="js_block_group_' . $this->_removeQuote($aArgs['location']) . '">';
				$sContent .= '<?php endif; ?>';

				$sContent .= '<?php if (defined(\'PHPFOX_IN_BLOCK_MODE\')): ?>';
				$sContent .= '<div class="js_sortable dnd_block_info" data-location="'. $this->_removeQuote($aArgs['location']) .'"><div class="js_sortable_title">Position '. $this->_removeQuote($aArgs['location']) .'<a href="#" class="add_new_block" data-location="'. $this->_removeQuote($aArgs['location']) .'" data-controller="<?php echo PHPFOX_IN_BLOCK_MODE;  ?>"></a></div>';
				$sContent .= '<?php $aBlocks = Phpfox::getService(\'admincp.block\')->getList(PHPFOX_IN_BLOCK_MODE, ' . $aArgs['location'] . '); ?>';
				$sContent .= '<?php foreach ((array) $aBlocks as $aBlock): ?>' . "\n";
				$sContent .= '<div class="block js_sortable_header" data-block-id="<?php echo $aBlock[\'block_id\']; ?>" data-block-type="<?php echo $aBlock[\'type_id\']; ?>"><?php echo $aBlock[\'title\']; ?></div>';
				$sContent .= '<?php endforeach; ?></div>';
				$sContent .= '<?php else: ?>';
			...
			case 'breadcrumb':
				$sContent = '<?php if (!$this->bIsSample): ?>';
				$sContent .= '<?php echo (defined(\'PHPFOX_IN_BLOCK_MODE\') ? \'\' : $this->getLayout(\'breadcrumb\')); ?>';
				$sContent .= '<?php endif; ?>';
				return $sContent;
				break;
			case 'search':
				$sContent = '<?php if (!$this->bIsSample): ?>';
				$sContent .= '<?php echo (defined(\'PHPFOX_IN_BLOCK_MODE\') ? \'\' : $this->getLayout(\'search\')); ?>';
				$sContent .= '<?php endif; ?>';
				return $sContent;
				break;
			case 'content':
				$sContent = '<?php if (defined(\'PHPFOX_IN_BLOCK_MODE2\')): ?>';
				$sContent .= '<div id="site_content">[CONTENT]</div>';
				$sContent .= '<?php else: ?>';
			...
			case 'menu':
				...
				break;	
			case 'menu_sub':
				return '<?php (defined(\'PHPFOX_IN_BLOCK_MODE\') ? \'[MENU]\' : Phpfox::getBlock(\'core.template-menusub\')); ?>';
				break;	
			case 'menu_footer':
				return '<?php (defined(\'PHPFOX_IN_BLOCK_MODE\') ? \'[FOOTER_MENU]\' : Phpfox::getBlock(\'core.template-menufooter\')); ?>';
				break;	
			case 'copyright':
				return '<?php (defined(\'PHPFOX_IN_BLOCK_MODE\') ? \'[COPYRIGHT]\' : Phpfox::getBlock(\'core.template-copyright\')); ?>';
				break;		
			case 'footer':
