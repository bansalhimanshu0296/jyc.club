...
{	
 	...
 	*/
	const VERSION = '4.0.0beta3';
	
	/**
	 * Product Code Name
	 * 
	 */	
	const CODE_NAME = 'Tetra';
	 ...
	 * @return bool
	 */
	public static function isApp($sModule)
	{
		return Phpfox::getLib('module')->isApp($sModule);
	}
	...
	{		
		...
		else 
		{
			...
			/*
			if ($sName == 'admincp.has_admin_access' && defined('MOXI9_PUBLIC_KEY') && $_SERVER['HTTP_HOST'] != 'localhost') {
				$bPass = false;
			} else {
			*/
	...
	{		
		...
		{		
			...
			else 
			{
				...
				$bIsAJAX = false;
				$sIsAjax = $oRequest->get('is_ajax_post');
				if(!empty($sIsAjax))
				{
					$bIsAJAX = true;
				}
				if(!empty($sQuery))
				{
					return $oModule->setController('search.index', array('is_ajax_post' => $bIsAJAX, 'q' => $sQuery));
				}
		...
		if (!PHPFOX_IS_AJAX_PAGE && !$oReq->get('is_ajax_pager'))
		{
			$oTpl->setImage(array(
		...
		if ($oReq->get('is_ajax_pager')) {
			Phpfox::getLib('phpfox.module')->getControllerTemplate();
			exit;
		}
		...
		if (!self::isAdminPanel() && !$oReq->get('is_ajax_pager'))
		{			
