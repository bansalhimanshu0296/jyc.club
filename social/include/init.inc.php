...
spl_autoload_register(function($class) {
	if (substr($class, 0, 6) == 'Unity_') {
		return;
	}

	if (substr($class, 0, 7) != 'Phpfox_') {
		$parts = explode('_', $class);
		require_once(PHPFOX_DIR_MODULE . strtolower($parts[0]) . PHPFOX_DS . 'include' . PHPFOX_DS . 'service' . PHPFOX_DS . strtolower($parts[2]) . '.class.php');
	}
});
...
if (file_exists(PHPFOX_DIR_SERVER_SETTING . 'license.php')) {
	require(PHPFOX_DIR_SERVER_SETTING . 'license.php');
}
