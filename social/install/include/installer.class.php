...
class Phpfox_Installer
{
	...
	public function __construct()
	{
		...
		{
			if (file_exists(PHPFOX_DIR_SERVER_SETTING . 'server.sett.php'))
			{
				require(PHPFOX_DIR_SERVER_SETTING . 'server.sett.php');
		...
		Phpfox::getLib('template')->assign('product_version', Phpfox::getVersion());
		Phpfox::getLib('template')->setHeader(array(
			'install.js' => 'static_script'
		));
	...
	public function run()
	{
		...
		if (!$this->_oReq->get('req2')) {
			$this->_oUrl->forward($this->_step('license'));
		}
	...
	private function _requirement()
	{
		...
		if (!$bNoLoadFail)
		{
			...
			if (!$oFile->isWritable(PHPFOX_DIR_THEME . 'frontend' . PHPFOX_DS)) {
				$aFileChecks['theme/frontend/'] = false;
			}

			if (!$oFile->isWritable(PHPFOX_DIR_FILE)) {
				$aFileChecks['file/'] = false;
			}
			...
			if (!count($aFileChecks)) {
				foreach ($aFiles as $sFile)
				{
					...
					$oFile->mkdir(PHPFOX_DIR . $sFile, true);
				}

				copy(PHPFOX_DIR_SETTING . 'server.sett.php.new', PHPFOX_DIR_SERVER_SETTING . 'server.sett.php');
		...
		if ($bIsPassed === true) {
			$this->_pass(($this->_bUpgrade ? 'update' : 'key'));
		}
	...
	private function _theme() {
		$this->_oTpl->setTitle('PHPFox Theme')->setBreadcrumb('PHPFox Theme')->assign('session_id', Phpfox::getLib('request')->get('sessionid'));

		if (!is_dir(PHPFOX_DIR_THEME . 'frontend' . PHPFOX_DS . 'unity' . PHPFOX_DS)) {
			mkdir(PHPFOX_DIR_THEME . 'frontend' . PHPFOX_DS . 'unity' . PHPFOX_DS);
		}

		$theme = file_get_contents('../static/jscript/theme.json');
		if (substr($theme, 0, 1) != '{') {
			exit('Importing theme failed. Please contact Moxi9 staff.');
		}

		$files = json_decode($theme);
		foreach ($files as $file => $data) {
			$parts = explode('/', $file);
			unset($parts[count($parts) - 1]);

			$path = PHPFOX_DIR_THEME . 'frontend' . PHPFOX_DS . 'unity' . PHPFOX_DS;
			$dir = $path . implode(PHPFOX_DS, $parts) . PHPFOX_DS;
			Phpfox::getLib('file')->mkdir($dir, true);
		...
		$this->_oTpl->assign(array(
				'sMessage' => 'Theme installed...',
				'sNext' => $this->_step('process')
			)
		);
	}
	...
	{
		exit('Upgrading to v4 is currently not supported until RC versions.');
	...
	{
		if (Phpfox::getLib('file')->isWritable(PHPFOX_DIR_SERVER_SETTING . 'server.sett.php'))
		{
			$sContent = file_get_contents(PHPFOX_DIR_SERVER_SETTING . 'server.sett.php');
			$sContent = preg_replace("/\\\$_CONF\['core.is_installed'\] = (.*?);/i", "\\\$_CONF['core.is_installed'] = true;", $sContent);
			if ($hServerConf = @fopen(PHPFOX_DIR_SERVER_SETTING . 'server.sett.php', 'w'))
			{
		...
		{
			$this->_db()->update(Phpfox::getT('theme'), array('is_default' => '0'), 'theme_id > 0');
			$this->_db()->update(Phpfox::getT('theme'), array('is_default' => '1'), 'folder = \'unity\'');

			$this->_db()->update(Phpfox::getT('theme_style'), array('is_default' => '0'), 'style_id > 0');
			$this->_db()->update(Phpfox::getT('theme_style'), array('is_default' => '1'), 'folder = \'unity\'');
