...
<div class="page_section_menu">
	<ul>
		<li class="active">
			<a href="#" class="js_custom_change_group" id="group_basic"><?php echo Phpfox::getPhrase('user.basic_information'); ?></a>
		</li>
<?php if (count ( $this->_aVars['aGroups'] )): ?>
<?php if (count((array)$this->_aVars['aGroups'])):  $this->_aPhpfoxVars['iteration']['groups'] = 0;  foreach ((array) $this->_aVars['aGroups'] as $this->_aVars['aGroup']):  $this->_aPhpfoxVars['iteration']['groups']++; ?>

				<li class="<?php if ($this->_aPhpfoxVars['iteration']['groups'] == count ( $this->_aVars['aGroups'] ) && Phpfox ::isModule('music') && ! Phpfox ::getUserParam('music.can_upload_music_public')): ?> last<?php endif; ?>"><a href="#" class="js_custom_change_group" id="group_<?php echo $this->_aVars['aGroup']['group_id']; ?>"><?php echo Phpfox::getPhrase($this->_aVars['aGroup']['phrase_var_name']); ?></a></li>
...
<?php endif; ?>
	...
	<div id="js_custom_public_message" class="public_message" style="margin-bottom:10px;">
		<a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl('profile'); ?>"><?php echo Phpfox::getPhrase('user.view_your_updated_profile'); ?></a>
	</div>

	<form method="post" action="<?php echo Phpfox::getLib('phpfox.url')->makeUrl('user.profile'); ?>"<?php if (! $this->_aVars['bIsEdit']): ?> onsubmit="<?php (($sPlugin = Phpfox_Plugin::get('user.template_controller_profile_form_onsubmit')) ? eval($sPlugin) : false); ?> $('#js_custom_public_message').hide(); $('#js_custom_submit_button').attr('disabled', true).addClass('disabled'); $('#js_custom_update_info').html($.ajaxProcess('<?php echo Phpfox::getPhrase('user.updating_profile', array('phpfox_squote' => true)); ?>')).show(); $(this).ajaxCall('custom.updateFields'); return false;"<?php endif; ?>>
<?php echo '<div><input type="hidden" name="' . Phpfox::getTokenName() . '[security_token]" value="' . Phpfox::getService('log.session')->getToken() . '" /></div>'; ?>
<?php if (isset ( $this->_aVars['iUserId'] )): ?>
		<div><input type="hidden" name="id" value="<?php echo $this->_aVars['iUserId']; ?>" /></div>
<?php endif; ?>
...
" size="30" />
		...
		<div class="table">
			<div class="table_left">
				<label for="postal_code"><?php echo Phpfox::getPhrase('user.zip_postal_code'); ?>:</label>
			</div>
			<div class="table_right">
				<input type="text" name="val[postal_code]" id="postal_code" value="<?php $aParams = (isset($aParams) ? $aParams : Phpfox::getLib('phpfox.request')->getArray('val')); echo (isset($aParams['postal_code']) ? Phpfox::getLib('phpfox.parse.output')->clean($aParams['postal_code']) : (isset($this->_aVars['aForms']['postal_code']) ? Phpfox::getLib('phpfox.parse.output')->clean($this->_aVars['aForms']['postal_code']) : '')); ?>
" size="10" />
			</div>
			<div class="clear"></div>
		</div>
...
<?php if (Phpfox ::getUserParam('user.can_edit_dob')): ?>
		<div class="table">
			<div class="table_left">
<?php if (Phpfox::getParam('core.display_required')): ?><span class="required"><?php echo Phpfox::getParam('core.required_symbol'); ?></span><?php endif;  echo Phpfox::getPhrase('user.date_of_birth'); ?>:
			</div>
			<div class="table_right">
...
<?php endif; ?>
<?php if (Phpfox ::getUserParam('user.can_edit_gender_setting')): ?>
		<div class="table">
			<div class="table_left">
				<label for="gender"><?php if (Phpfox::getParam('core.display_required')): ?><span class="required"><?php echo Phpfox::getParam('core.required_symbol'); ?></span><?php endif;  echo Phpfox::getPhrase('user.gender'); ?>:</label>
			</div>
			<div class="table_right">
...
<?php if (Phpfox ::getParam('user.enable_relationship_status') && Phpfox ::getUserParam('custom.can_have_relationship') && isset ( $this->_aVars['aRelations'] ) && ! empty ( $this->_aVars['aRelations'] )): ?>
		<div class="table">
			<div class="table_left">
<?php echo Phpfox::getPhrase('custom.relationship_status'); ?>
			</div>
			<div class="table_right">
				<select name="val[relation]" id="relation" onchange="$Behavior.displayRelationshipChange()">
<?php if (count((array)$this->_aVars['aRelations'])):  foreach ((array) $this->_aVars['aRelations'] as $this->_aVars['aRelation']): ?>
						<option value="<?php echo $this->_aVars['aRelation']['relation_id']; ?>" <?php if (isset ( $this->_aVars['aForms']['relation_id'] ) && $this->_aVars['aForms']['relation_id'] == $this->_aVars['aRelation']['relation_id']): ?> selected="selected"<?php endif; ?>><?php echo Phpfox::getPhrase($this->_aVars['aRelation']['phrase_var_name']); ?> </option>
<?php endforeach; endif; ?>
				</select>
				
				<script type="text/javascript">
					var aRelationshipChange = <?php echo $this->_aVars['sJsArray']; ?>;
					
<?php if (isset ( $this->_aVars['aForms']['relation_id'] )): ?>
						$Behavior.setRelationship = function(){ $('#relation').val(<?php echo $this->_aVars['aForms']['relation_id']; ?>); }
<?php endif; ?>
				</script>
				
				<span id="relation_with">
					<span class="js_relation_with_message" id="relation_with_message_to" style="display:none;"><?php echo Phpfox::getPhrase('user.to'); ?>:</span>
					<span class="js_relation_with_message" id="relation_with_message_with" style="display:none;"><?php echo Phpfox::getPhrase('user.with'); ?>:</span>
						<div class="edit_friend_relation">
										<span id="js_custom_search_friend"></span>	
										<span id="sFriendImage">
<?php if (isset ( $this->_aVars['aForms']['with_user'] ) && isset ( $this->_aVars['aForms']['with_user']['user_image'] ) && ! empty ( $this->_aVars['aForms']['with_user'] ) && $this->_aVars['aForms']['with_user']['with_user_id'] > 0): ?>
<?php echo Phpfox::getLib('phpfox.image.helper')->display(array('user' => $this->_aVars['aForms']['with_user'],'suffix' => '_50_square','max_height' => 50)); ?>
<?php endif; ?>
										</span>
<?php if (isset ( $this->_aVars['aForms']['with_user']['user_id'] ) && ! empty ( $this->_aVars['aForms']['with_user']['user_id'] )): ?>
											<input type="hidden" id="relation_with_input_hidden" name="val[relation_with]" value="<?php echo $this->_aVars['aForms']['with_user']['user_id']; ?>">
<?php endif; ?>
										<div id="js_custom_search_friend_placement"></div>
										
										<input type="hidden" name="val[previous_relation_with]" value="<?php if (isset ( $this->_aVars['aForms']['with_user']['user_id'] )):  echo $this->_aVars['aForms']['with_user']['user_id'];  else: ?>0<?php endif; ?>">
										<input type="hidden" name="val[previous_relation_type]" value="<?php if (isset ( $this->_aVars['aForms']['relation_id'] )):  echo $this->_aVars['aForms']['relation_id'];  else: ?>0<?php endif; ?>">
<?php if (isset ( $this->_aVars['aForms']['with_user']['status_id'] ) && $this->_aVars['aForms']['with_user']['status_id'] == 1): ?>
										<div style="margin-left:60px;"><?php echo Phpfox::getPhrase('user.pending_confirmation'); ?></div>
<?php endif; ?>
						</div>
						<?php echo '
						<script type="text/javascript">
						$Behavior.profileSearchFriends = function()
						{
							$Core.searchFriends({
								...
								\'max_search\': 10,
								\'input_name\': \'friends\',
								\'default_value\': ';  if (isset ( $this->_aVars['aForms']['with_user'] ) && $this->_aVars['aForms']['with_user']['with_user_id'] > 0): ?>'<?php echo $this->_aVars['aForms']['with_user']['full_name']; ?>' <?php else: ?> '<?php echo Phpfox::getPhrase('user.search_friends_by_their_name'); ?>'<?php endif;  echo ',
...
<?php if (Phpfox ::isModule('forum')): ?>
		<div style="display:none">
			<div class="separate"></div>
			<div class="table">
				<div class="table_left">
<?php echo Phpfox::getPhrase('user.forum_signature'); ?>:
				</div>
				<div class="table_right">
					<textarea cols="40" rows="5" id="signature" name="val[signature]"><?php echo $this->_aVars['aForms']['signature']; ?></textarea>
				</div>
				<div class="clear"></div>
			</div>
		</div>
...
<?php if (Phpfox ::isModule('feed') && Phpfox ::getParam('feed.timeline_optional')): ?>
		<div class="separate"></div>		
		<div class="table">
			<div class="table_left">
<?php echo Phpfox::getPhrase('user.timeline'); ?>:
			</div>
			<div class="table_right">
...
if (isset($this->_aVars['aForms'])
 && is_numeric('use_timeline') && in_array('use_timeline', $this->_aVars['aForms']))
							
{
								echo ' selected="selected" ';
							}

							if (isset($aParams['use_timeline'])
								&& $aParams['use_timeline'] == '0')
...

							{
								...
								if (isset($this->_aVars['aForms']['use_timeline'])
									&& !isset($aParams['use_timeline'])
									&& $this->_aVars['aForms']['use_timeline'] == '0')
								{
								 echo ' selected="selected" ';
								}
								else
							...
							?>
><?php echo Phpfox::getPhrase('user.no'); ?></option>
					<option value="1"<?php $aParams = (isset($aParams) ? $aParams : Phpfox::getLib('phpfox.request')->getArray('val'));
							...
							
{
...

							{
								...
								if (isset($this->_aVars['aForms']['use_timeline'])
									&& !isset($aParams['use_timeline'])
									&& $this->_aVars['aForms']['use_timeline'] == '1')
								{
								 echo ' selected="selected" ';
								}
								else
							...
							?>
><?php echo Phpfox::getPhrase('user.yes'); ?></option>
				</select>
			</div>
			<div class="clear"></div>
		</div>			
...
<?php if (Phpfox ::getParam('profile.allow_user_select_landing')): ?>
<?php if (Phpfox ::isModule('feed') && ! Phpfox ::getParam('feed.timeline_optional')): ?>
		<div class="separate"></div>		
<?php endif; ?>
		<div class="table">
			<div class="table_left">
<?php echo Phpfox::getPhrase('user.landing_page'); ?>:
			</div>
			<div class="table_right">
				<select name="val[landing_page]">
					<option value="wall"<?php $aParams = (isset($aParams) ? $aParams : Phpfox::getLib('phpfox.request')->getArray('val'));
...
if (isset($this->_aVars['aForms'])
 && is_numeric('landing_page') && in_array('landing_page', $this->_aVars['aForms']))
							
{
								echo ' selected="selected" ';
							}

							if (isset($aParams['landing_page'])
								&& $aParams['landing_page'] == 'wall')
...

							{
								...
								if (isset($this->_aVars['aForms']['landing_page'])
									&& !isset($aParams['landing_page'])
									&& $this->_aVars['aForms']['landing_page'] == 'wall')
								{
								 echo ' selected="selected" ';
								}
								else
							...
							?>
><?php echo Phpfox::getPhrase('user.wall'); ?></option>
					<option value="info"<?php $aParams = (isset($aParams) ? $aParams : Phpfox::getLib('phpfox.request')->getArray('val'));
							...
							
{
...

							{
								...
								if (isset($this->_aVars['aForms']['landing_page'])
									&& !isset($aParams['landing_page'])
									&& $this->_aVars['aForms']['landing_page'] == 'info')
								{
								 echo ' selected="selected" ';
								}
								else
							...
							?>
><?php echo Phpfox::getPhrase('user.profile_info'); ?></option>
				</select>
			</div>
			<div class="clear"></div>
		</div>		
...
<?php if (count((array)$this->_aVars['aSettings'])):  foreach ((array) $this->_aVars['aSettings'] as $this->_aVars['aSetting']): ?>
			<div class="table js_custom_groups js_custom_group_<?php echo $this->_aVars['aSetting']['group_id']; ?>" style="display:none;">
				<div class="table_left">
<?php if ($this->_aVars['aSetting']['is_required']):  if (Phpfox::getParam('core.display_required')): ?><span class="required"><?php echo Phpfox::getParam('core.required_symbol'); ?></span><?php endif;  endif;  echo Phpfox::getPhrase($this->_aVars['aSetting']['phrase_var_name']); ?>:
				</div>
				<div class="table_right">
					<?php
...
<?php if (Phpfox ::getUserParam('custom.can_add_custom_fields')): ?>
<?php echo Phpfox::getPhrase('user.no_custom_fields_have_been_added'); ?>
				<ul class="action">
					<li><a href="<?php echo Phpfox::getLib('phpfox.url')->makeUrl('admincp.custom.add'); ?>"><?php echo Phpfox::getPhrase('user.add_a_new_custom_field'); ?></a></li>
				</ul>
<?php endif; ?>
		</div>
<?php endif; ?>
<?php (($sPlugin = Phpfox_Plugin::get('user.template_controller_profile_form')) ? eval($sPlugin) : false); ?>
	<div class="table_clear">
		<input type="submit" value="<?php echo Phpfox::getPhrase('user.update'); ?>" class="button" id="js_custom_submit_button"> <span id="js_custom_update_info"></span>
	</div>
