...
If you disable it extra checks will be added to try to preserve the original name of the file. If an item with the same name already exists the new one will have a number added at the end.</info>',
  ...
  'setting_keep_files_in_server' => '<title>Keep Files In Server</title><info>Set this to TRUE to keep original files on the server.</info>',
...
More information on how to get an API key can be found <a href="https://developers.google.com/maps/documentation/javascript/tutorial#api_key">here</a>.</info>',
  'setting_activity_feed_line_breaks' => '<title>Activity Feed Line Breaks</title><info>This feature controls how many line breaks to show in activity feed posts and comments. To enable this feature simply add a number higher than 0.</info>',
  'setting_include_master_files' => '<title>JS/CSS Include Master Files</title><info>This setting is designed to create 1 JS and 1 CSS master file so it would only be loaded once by the browser. Then when a user visits a page it will load another file that is specific to that page only. The file will be a lot smaller since it does not have any of the master data.</info>',
  ...
  'setting_defer_loading_images' => '<title>Defer Photos</title><info>Load photos from the Photo module after the site has loaded.</info>',
  ...
  'setting_controllers_to_load_delayed' => '<title>Load sections via AJAX</title><info>You can define an array of controllers that will be loaded after the site has been loaded via AJAX.</info>',
  'setting_super_cache_system' => '<title>Global Caching</title><info>Global caching setting. Enable to turn on caching system on several areas. Drops database queries.</info>',
  'setting_store_only_users_in_session' => '<title>Only Users in Session Table</title><info>Only store users in the session table. Keeps bots and guests out from this table.</info>',
  'setting_push_jscss_to_cdn' => '<title>Upload CSS/JS to CDN</title><info>Upload minified CSS/JS to CDN if minified files is enabled. Note you will still need to manually upload your themes images. This only works for CSS and JavaScript files.</info>',
  ...
  'setting_force_404_check' => '<title>Force 404 Check</title><info>Enable to force 404 checks on sub-pages that do not exist. This works in selective sections.</info>',
  'setting_enable_html_purifier' => '<title>HTML PURIFIER</title><info>If enabled we will use a 3rd party library to assist in parsing content submitted by your users. Only enable this if you allow HTML on your site. To learn more about this product check out:
http://htmlpurifier.org/

You can find additional settings in the file:
include/setting/htmlpurifier.sett.php.new</info>',
  ...
  'setting_auth_user_via_session' => '<title>Authenticate Users via Database Session Log</title><info>If you enable this option we will authenticate users via a database session log.</info>',
  ...
  'setting_check_body_for_text' => '<title>HTML Body Check</title><info>Enable to check HTML body for specific text.</info>',
  'setting_check_body_regex' => '<title>HTML Body Check REGEX</title><info>Add your custom REGEX to check the HTML body.</info>',
  'setting_check_body_offline_message' => '<title>HTML Body Check Failed Message</title><info>If the REGEX check fails, add a message we should display.</info>',
  'setting_check_body_header' => '<title>HTML Body Check Failed Response</title><info>Custom header to send the browser in case the HTML body check REGEX fails.</info>',
  'setting_html_purifier_allowed_html' => '<title>HTML PURIFIER Allowed HTML</title><info>Comma separated list of HTML elements and attributes HTML Purifier should allow.

To learn more on how this setting works check out:
http://htmlpurifier.org/live/configdoc/plain.html#HTML.Allowed</info>',
  'setting_html_purifier_allowed_iframes' => '<title>HTML PURIFIER Allowed IFRAMES REGEX</title><info>REGEX of sites HTML Purifier should allow when users use HTML IFRAMES.</info>',
